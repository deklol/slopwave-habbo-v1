...
public class Room {
    ...
    public void loadRoom(final Player player, Position door, int rotation) {
        ...
        this.removeExistingRoomSession(player);
    ...
    private void removeExistingRoomSession(Player player) {
        if (this.entities == null || player == null || player.getDetails() == null) {
            return;
        }

        int userId = player.getDetails().getID();
        List<Entity> existingSessions = new ArrayList<Entity>();
        ...
        for (Entity entity : this.entities) {
            ...
            Player existingPlayer = (Player) entity;

            if (existingPlayer.getDetails() == null) {
                continue;
            }

            if (existingPlayer.getDetails().getID() == userId) {
                existingSessions.add(existingPlayer);
            }
        }

        for (Entity entity : existingSessions) {
            this.entities.remove(entity);

            if (entity instanceof Player && entity != player) {
                Player existingPlayer = (Player) entity;
                existingPlayer.getRoomUser().dispose();
                existingPlayer.kick();
            }
        }
    }
